...
const GenericTable = ({colName, columns, rowData, header, loading, exportTitle}: IGenericTableProps) => {
    ...
    //     downloadExcel({
    //         fileName: formatFilename(exportTitle),
    //         sheet: formatFilename(exportTitle),
    //         tablePayload: {
    //             header,
    //             body: rowData,
    //         },
        ...
        if (type === 'csv') {
            ...
            records.map((item: any) => {
                columns.map((d: any, index: any) => {
                    ...
                    let val = getNestedValue(item, d) ? getNestedValue(item, d) : '';
                    result += val;
                });
                result += linedelimiter;
            });
            ...
            var rowhtml = '<p>' + formatFilename(filename) + '</p>';
            rowhtml +=
            ...
            winPrint.document.write('<title>'+formatFilename(filename)+'</title>' + rowhtml);
    ...
    const formatFilename = (filename: string) => {
        ...
        const reportType = parts.slice(0, -1).join(' ')
            .replace(/\b\w/g, l => l.toUpperCase());
        const timestamp: any = parts.pop();
        const date = new Date(parseInt(timestamp, 10));
        const formattedDate = `${date.getMonth() + 1}-${date.getDate()}-${date.getFullYear()}`;
        return `${reportType} ${formattedDate}`;
    }
        ...
        <>
            <div className="mb-4.5 flex flex-col justify-between gap-5 md:flex-row md:items-center">
                <div className="flex flex-wrap items-center">
                    ...
                    <button type="button" onClick={() => exportTable('txt')}
                            className="btn btn-primary btn-sm m-1">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                             xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 ltr:mr-2 rtl:ml-2">
                            <path
