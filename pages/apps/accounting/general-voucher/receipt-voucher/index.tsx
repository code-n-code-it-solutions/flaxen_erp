...
const Index = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('GRV List'));
        setAuthToken(token);
        ...
        <div className="flex flex-col gap-5">
            <PageHeader
                ...
                leftComponent={{
                    addButton: {
                        ...
                        type: 'link',
                        text: 'New',
                        link: '/apps/accounting/general-voucher/receipt-voucher/create'
                    },
                    title: 'General Receipt Voucher',
                    showSetting: true
                }}
                rightComponent={true}
                showSearch={true}
                buttonActions={{
                    ...
                    print: () => router.push('/apps/accounting/general-voucher/receipt-voucher/print/' + selectedRows.map(row => row.id).join('/')),
            ...
            <div>
                <AgGridComponent
                    ...
                    onRowClicked={(params) => {
                        checkPermission(menus.map((plugin: any) => plugin.menus).flat(), activeMenu.route, ActionList.VIEW_DETAIL, AppBasePath.General_Receipt_Voucher) &&
                        router.push(`/apps/accounting/general-voucher/receipt-voucher/view/${params.data.id}`);
                    }}
                />
            </div>
        </div>
