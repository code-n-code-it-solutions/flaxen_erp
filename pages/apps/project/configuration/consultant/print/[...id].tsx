...
import { clearClientState, getClientsForPrint } from '@/store/slices/projects/clientSlice';

const Print = () => {
    const dispatch = useAppDispatch();
    const { clientsForPrint: contents, loading } = useAppSelector((state) => state.client);
    ...
    useEffect(() => {
        ...
        if(router.query.id) {
            dispatch(getClientsForPrint({ ids: router.query.id, type: 'detail'}));
        }
    }, [router.query.id]);
