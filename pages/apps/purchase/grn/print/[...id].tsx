...
import { clearGoodReceiveNoteState, getGRNsForPrint } from '@/store/slices/goodReceiveNoteSlice';

const Print = () => {
    const dispatch = useAppDispatch();
    const { loading, GRNsForPrint: contents } = useAppSelector(state => state.goodReceiveNote);
    ...
    useEffect(() => {
        ...
        if (router.query.id) {
            dispatch(getGRNsForPrint({ ids: router.query.id, type: 'detail' }));
        }
    }, [router.query.id]);
