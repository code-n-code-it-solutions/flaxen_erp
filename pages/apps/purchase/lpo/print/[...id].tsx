...
import { clearLocalPurchaseOrderState, getLPOsForPrint } from '@/store/slices/localPurchaseOrderSlice';

const Print = () => {
    const dispatch = useAppDispatch();
    const { loading, LPOsForPrint:contents } = useAppSelector(state => state.localPurchaseOrder);
    ...
    useEffect(() => {
        ...
        if(router.query.id) {
            dispatch(getLPOsForPrint({ ids: router.query.id, type: 'detail'}));
        }
    }, [router.query.id]);
