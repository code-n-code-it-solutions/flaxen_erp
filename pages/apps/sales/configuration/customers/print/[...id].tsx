...
import { clearCustomerState, getCustomersForPrint } from '@/store/slices/customerSlice';

const Print = () => {
    const dispatch = useAppDispatch();
    const { customersForPrint: contents, loading } = useAppSelector((state) => state.customer);
    ...
    useEffect(() => {
        ...
        if(router.query.id) {
            dispatch(getCustomersForPrint({ ids: router.query.id, type: 'detail'}));
        }
    }, [router.query.id]);
