...
import { clearDeliveryNoteState, getDeliveryNotesForPrint } from '@/store/slices/deliveryNoteSlice';

const Print = () => {
    const dispatch = useAppDispatch();
    const { deliveryNotesForPrint: contents, loading } = useAppSelector((state) => state.deliveryNote);
    ...
    useEffect(() => {
        ...
        if(router.query.id) {
            dispatch(getDeliveryNotesForPrint({ ids: router.query.id, type: 'detail'}));
        }
    }, [router.query.id]);
